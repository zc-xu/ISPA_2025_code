...
\else
  ...
  \newcommand{\rev}[1]{#1}
\fi
\newcommand{\revcolor}{\ifshowrevisions\color{blue}\fi}
...
xfdi@bjtu.edu.cn}
\and
\IEEEauthorblockN{\rev{4\textsuperscript{th}} Jingxin Su}
\IEEEauthorblockA{\textit{School of Software Engineering} \\
\textit{Beijing Jiaotong University}\\
Beijing, China \\
jxsu@bjtu.edu.cn}
\and
\IEEEauthorblockN{\rev{5\textsuperscript{th}} Xiaoping Che}
\IEEEauthorblockA{\textit{School of Software Engineering} \\
\textit{Beijing Jiaotong University}\\
Beijing, China \\
xpche@bjtu.edu.cn}
...
The remainder of this paper is organized as follows. In Section II, we review related research. In Section III, we \rev{model} the problem. In Section IV, we propose the server deployment and service provisioning algorithms. In Section V, we \rev{present the comparative experimental results}. Finally, in Section VI, we conclude the paper.
...
\begin{tabular}{|>{\centering\arraybackslash}m{2cm}|>{\raggedright\arraybackslash}m{6cm}|}
    ...
    $p_j$ & Deployment cost of server $m_j$ \\ \hline
    \rev{$a_{ij}$} & \rev{Indicator whether user $u_i$ is served by $m_j$.}\\ \hline
    \multirow{2}{*}{\rev{$y_{jk}$}} & \rev{Indicator whether server $m_j$ is deployed} \\ 
    & \rev{at base station $b_k$.}\\ \hline
    \rev{$a_{ijw}$} & \rev{Indicator whether user $u_i$ is served by $s_w$ on $m_j$.} \\ \hline
    \rev{$z_{jw}$} & \rev{Indicator whether server $m_j$ provisions service $s_w$.} \\ \hline
    $C_{max}$ & Maximum allowed total server deployment cost \\ \hline
    $\mathcal{K}^c$ & Maximum number of deployable server nodes under $C_{max}$ \\ \hline
    % $r$ & Optimal service coverage radius of each base station \\ \hline
    $\sigma_{min}$ & Minimum required user density \\ \hline
   $\mathcal{K}^r$ & Number of base stations with local user density exceeds $\sigma_{min}$ \\ \hline
    $k$ & Number of servers actually deployed \\ \hline
    $\alpha$ & Unit price of communication bandwidth \\ \hline
    \rev{$\beta_{iw}$} & \rev{Number of hops between user $u_i$ and server $m_j$ providing $s_w$.} \\ \hline
    $\tau(u_i \rightarrow m_j)$ & Distance between user $u_i$ and server $m_j$ \\ \hline
    $V_j$ & Service capacity of server $m_j$ \\ \hline
...
& \rev{\sum_{w \in \mathcal{S}} z_{jw}v_w \leq V_j, \quad \sum_{w \in S_j} z_{jw}\varsigma_w \leq \mu_j,} \,\label{eq5} \\
& \rev{a_{ijw}\left({R^{\textbf{c}}_{iw}}/{\mu_j} + ({R^{\textbf{t}}_{iw}}/{r_{ij}})\beta_{iw}\right) \leq \widetilde{D_i}, \quad \forall i \in \mathcal{U},} \label{eq6} \\
& \rev{y_{jk}, z_{jw}, a_{ijw} \in \{0,1\}, \quad \forall i \in \mathcal{U}, \, \forall j \in \mathcal{M}.}\label{eq7}
\end{align}  
\begin{revision}
...
\begin{figure*}[!t]
 \centering
    ...
    \caption{\rev{Overall architecture of MOS\textsuperscript{2}.}}
    \label{fig:algo}
\end{figure*}

\section{\rev{Algorithm Design}}
...
\begin{algorithm}[!t]
    ...
    \KwOut{$\mathcal{S}$, $C$}
    \rev{$\mathcal{S} \leftarrow \textbf{RandomSelect}(\mathcal{M},k)$}\;
...
\begin{algorithm}[!t]
    ...
    \KwIn{$\mathcal{M}$, $\mathcal{U}$, $\mathcal{S}$, $k$, $G$, $N$, $V_j$, $\alpha$, $\beta$, \rev{$\varpi_j$}}
    \KwOut{$\mathcal{P}^*$}
    ...
    \ForEach{$X \in \mathcal{P}$}{
        ...
        \rev{For each row $j$, randomly deactivate selected entries while $\sum_w X_{jw}>V_j$}\;
    ...
    \For{$g \leftarrow 1$ \KwTo $\mathrm{G}$}{
        ...
        \ForEach{$X' \in \mathcal{P}'$}{
            \rev{Round and reshape $X'$; repair each violating row as above; evaluate $X'$};% as above\;
        }
...
\begin{figure*}[!t]
 ...
 %    \vspace{-5pt}
	...
	\caption{\rev{Stage-I server-deployment performance under different server and user scales.}}
	\label{fig2}
    \vspace{-7pt}
\end{figure*}
...
\begin{figure}[!t]
	...
	\caption{\rev{Stage-I cost analysis under varying server counts and CLS iterations.}}
	\label{fig_compare}
    \vspace{-7pt}
\end{figure}
\begin{figure*}[!t]
	...
	\caption{\rev{Service-provisioning performance with 10 deployed servers and increasing user populations.}}
	\label{fig4}
\end{figure*}
...
\begin{figure}[!t]
	...
	\caption{\rev{Hybrid initialization for candidate services on Server~0.}}
...
\begin{figure*}[!t]
	...
	\caption{\rev{Service-provisioning performance with 130 users and increasing numbers of deployed edge servers.}}
	\label{fig6}
\end{figure*}
...
\begin{figure*}
	...
	\caption{\rev{Pareto fronts for Stage-II service provisioning with 130 users.}}
	\label{fig7}
    \vspace{-7pt}
\end{figure*}
...
\rev{In Stage~I, CLS is compared with four baseline methods:}
%Local-Search对比算法
\begin{itemize}
...
%NSGA-II对比算法
\begin{itemize}
    \item \rev{\textbf{NSGA-II Provision (NS-P)}: The initial population is generated randomly.}
    ...
    \item \rev{\textbf{Deep Q-Network Provision (DQN)}: A Q-network sequentially selects a service or an empty action for each server slot using demand, deployment cost, and a cost--delay preference. The resulting deployment is evaluated by the same objectives as the other methods.}
\end{itemize}
...
\begin{figure}[!t]
    ...
    \caption{\rev{Initialization sensitivity of CLS under two server/user settings.}}
    \label{fig:cls_init}
\end{figure}
...
\section{Conclusion}
 This paper presents MOS², a two-stage multi-objective framework for joint cost-delay optimization in mobile edge networks,
  ...
  decisions. \rev{Future work will extend the framework with reliability-aware QoS constraints for packet loss, link availability, and service interruption, together with learning-based demand prediction and multi-domain coordination.}
...
\bibitem{b33} \rev{C. Feng, M. Yang, Z. Jing, T. Q. S. Quek, and M. Mei, ``Latency-Aware Service Deployment and Peer Offloading: A Long-Term Optimization Framework for Satellite Edge Computing,'' IEEE Internet of Things Journal, vol. 13, no. 1, pp. 405--417, 2026.}

\bibitem{b34} \rev{M. Kato, T. K. Rodrigues, and S. Verma, ``Novel Breakout Local Search for Offloading Tasks in Multi-Tiered Cloud Environment Considering Transmission and Processing,'' in Proc. 2025 9th IEEE International Conference on Network Intelligence and Digital Content (IC-NIDC), 2025, pp. 269--274.}

\bibitem{b35} \rev{Z. Jing, Q. Yang, M. Qin, J. Li, and K. S. Kwak, ``Long-Term Max--Min Fairness Guarantee Mechanism for Integrated Multi-RAT and MEC Networks,'' IEEE Transactions on Vehicular Technology, vol. 70, no. 3, pp. 2478--2492, 2021.}

\bibitem{b36} \rev{Z. Jing, X. Wang, Q. Yang, M. Mei, and Y. Wu, ``Dynamic Energy Cost Conservation for Distributed Edge Clouds Utilizing Online Mini-Batch Learning,'' in Proc. IEEE 34th Annual International Symposium on Personal, Indoor and Mobile Radio Communications (PIMRC), 2023, pp. 1--6.}
