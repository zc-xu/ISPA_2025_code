...
xfdi@bjtu.edu.cn}
\and
\IEEEauthorblockN{4\textsuperscript{rd} Jingxin Su}
\IEEEauthorblockA{\textit{School of Software Engineering} \\
\textit{Beijing Jiaotong University}\\
Beijing, China \\
jxsu@bjtu.edu.cn}
\and
\IEEEauthorblockN{5\textsuperscript{rd} Xiaoping Che}
\IEEEauthorblockA{\textit{School of Software Engineering} \\
\textit{Beijing Jiaotong University}\\
Beijing, China \\
xpche@bjtu.edu.cn}
...
%%会议版本略去的部分，留待期刊版本补充。
To clarify this problem, we turn to the example provided in Figure~\ref{fig1}. In this illustration, five base stations are depicted, with edge servers deployed in areas $3$ and $4$, where the red dashed regions highlight the base stations where edge servers are located. Services that encompass varied computational tasks are represented by distinct-colored blocks ($s_{1}$ to $s_{6}$), while users of the same color denote the corresponding service requests. 
Each service replica serves a single user and generates income. Due to the limited capacity of edge servers, only a subset of services can be deployed locally, while others require a decision on whether it is worthwhile to provision on the edge servers. If no matching service replica is available on the server, the request will be forwarded to another server where the replica is stored, resulting in additional cost. Taking area 4 as an example, it is capable of provisioning services $s_2$ , $s_3$ and $s_5$. However, when facing requests for other services originating from different areas, it has to rely on service routing through intermediate servers, which incurs additional transmission overhead, thus giving rise to some issues and challenges:
The server in area 4 could deal with the service requests from users associated with $s_2$ , $s_3$ and $s_5$, e.g., users from areas 4 and 5. 
However, when dealing with service requests from $s_{1}$, $s_{4}$ and $s_{6}$, additional transmission cost is incurred, thus giving rise to some issues and challenges: 
...
(ii) Designing an effective service provisioning strategy under constrained server capacity while jointly optimizing provisioning cost and users' delay remains a significant challenge. For example, in area 3, the deployed server can only host three service instances, which cannot satisfy all user requests simultaneously. Two services, $s_{2}$ and $s_{5}$, have already been provisioned, and the remaining slot must be assigned to one of the candidate services in $\{s_{2}, s_{3}, s_{4}, s_{6}\}$. Since the selection of services to deploy directly affects the transmission cost, which in turn influences service delivery latency and ultimately user satisfaction, the provisioning process must consider the user request frequency, service provision cost, and the potential revenue of each candidate service. Users experiencing low latency are more likely to reuse the service, thereby increasing the operator’s revenue. Therefore, devising an optimal combination of server deployment and service provisioning strategies is crucial for maximizing the profit of mobile network operators.
...
The remainder of this paper is organized as follows. In Section II, we review related research. In Section III, we modeled the problem. In Section IV, we propose the server deployment and service provisioning algorithms. In Section V, we illustrate the comparative experimental evaluation results. Finally, in Section VI, we conclude the paper.
...
The server deployment cost refers to the inherent cost incurred when deploying an edge server co-located with a base station. We assume that the servers are heterogeneous and that the deployment cost of the server $m_j$ is denoted by $p_j$. Here, we introduce a binary variable $x_{jk}$, where $x_{jk} = 1$ if an edge server $m_j$ is deployed at base station $b_k$, and $0$ otherwise. Accordingly, the deployment cost of server $m_j$ is $C_{d}(m_j)=x_{jk}\cdot p_j$. 
...
The service provisioning cost is represented by the set $\mathcal{SC} = \{sc_{w}\}$, where $sc_{w}$ denotes the provisioning cost of the $w$-th service. Let $x_{jw}$ be a binary variable, where $x_{jw} = 1$ if an instance of service $s_{w}$ is deployed on edge server $w_{j}$, and $x_{jw} = 0$ otherwise. The corresponding deployment cost is denoted by $sc_w$. Furthermore, let $x_{ijw}$ be a binary variable indicating whether user $u_i$ is served by service $s_w$ deployed on server $w_j$, such that $x_{ijw} = 1$ if the assignment exists, and $0$ otherwise. The total provisioning cost of a server is determined by the sum of the provisioning costs of all services deployed on it, where $C_{p}(m_j) = \sum_{sc_w \in \mathcal{SC}} x_{jw} \cdot sc_w$. %Consequently, the total cost of service provisioning is defined as $C_{\text{prov}} = \sum_{sc_w \in \textbf{SC}} \sum_{j \in M} x_{jw} \cdot sc_w$. 
...
\begin{tabular}{|>{\centering\arraybackslash}m{2cm}|>{\raggedright\arraybackslash}m{6cm}|}
    ...
    $p_j$ & Deployment cost of server $m_j$ \\ \hline
    $x_{ij}$ & Indicator whether user $u_i$ is served by $m_j$.\\ \hline
    \multirow{2}{*}{$x_{jk}$} & Indicator whether server $m_j$ is deployed \\ 
    & at base station $b_k$.\\ \hline
    $x_{ijw}$ & Indicator whether user $u_i$ is served by $s_w$ on $m_j$. \\ \hline
    $x_{jw}$ & Indicator whether server $m_j$ deploys service $s_w$ \\ \hline
    $C_{max}$ & Maximum allowed total server deployment cost \\ \hline
    $\mathcal{K}^c$ & Maximum number of deployable server nodes under $C_{max}$ \\ \hline
    % $r$ & Optimal service coverage radius of each base station \\ \hline
    $\sigma_{min}$ & Minimum required user density \\ \hline
   $\mathcal{K}^r$ & Number of base stations with local user density exceeds $\sigma_{min}$ \\ \hline
    $k$ & Number of servers actually deployed \\ \hline
    $\alpha$ & Unit price of communication bandwidth \\ \hline
    $\beta_{iw}$ & Number of hops between user $u_i$ and server $s_w$ \\ \hline
    $\tau(u_i \rightarrow m_j)$ & Distance between user $u_i$ and server $m_j$ \\ \hline
    $V_j$ & Service capacity of server $m_j$ \\ \hline
...
In this system, user latency occurs both during computation and communication. We define $D_{p}(m_j)$ as the computational latency associated with user requests processed by edge server $m_j$. This latency depends on the resource requirements of the requested services and the computing capabilities of the servers, and is given by $D_{p}(m_j) = \sum_{i \in U} x_{ijw} \cdot {R^{\textbf{c}}_{iw}}/{\mu_j}$, where $\mu_j$ denotes the computing capacity of edge server $m_j$, $R^{\textbf{c}}_{iw}$ represents the computational resource demand of the service $s_w$ requested by the user $u_i$, and $x_{ijw}$ indicates whether $u_i$ is served by $s_w$ on $m_j$. 
%这里假设的是并行处理的，总的计算资源请求不能超过总core数

In addition, communication latency refers to data transmission between users and edge servers, which occurs when a user and the corresponding service are deployed in different regions. It is influenced by the propagation and transmission times of the network, where the propagation delay is determined by the distance $\tau(u_i \rightarrow m_j)$ between the user $u_i$ and the edge server $m_j$ hosting service $s_i$. Here, we define the maximum transmission rate $r_{ij}$ between the user $u_i$ and the edge server $m_j$ as $r_{ij} = B_{ij} \cdot \log_2 \left( 1 + {\gamma \cdot g_{ij}}/{\mathcal{N}} \right)$~\cite{b28,b31, b32}, where $B_{ij}$ denotes the channel bandwidth, $\gamma$ is the transmission power of the the user device, $\mathcal{N}$ is the noise power, and $g_{ij}$ represents the channel gain between $u_i$ and $m_j$. The channel gain $g_{ij}$ is modeled as $g_{ij} = 127 + 30 \cdot \log \left( \tau_{(u_i \rightarrow m_j)} \right)$. For each user, the data transmission time is determined by the size of the data, denoted as $R^{\textbf{t}}_{iw}$. Due to the distribution of edge servers and the provisioning of services, a requested service from user $u_i$ may not be directly available on the nearest server, leading to forward transmissions through intermediate servers, therefore, we introduce a per-hop communication delay coefficient to capture this effect, analogous to the per-unit distance cost coefficient defined above. Consequently, the communication latency of the users is given by $D_{c}(m_j) = \sum_{i \in U} x_{ijw} \cdot ({R^{\textbf{t}}_{iw}}/{r_{ij}}) \cdot \beta_{iw}$. 
...
& \sum_{w \in \textbf{S}} x_{jw}\cdot v_w \leq V_j, \quad \sum_{w \in S_j} x_{jw}\cdot \varsigma_{w} \leq \mu_j, \,\label{eq5} \\
& x_{ijw} (\cdot {R^{\textbf{c}}_{iw}}/{\mu_j} + ({R^{\textbf{t}}_{iw}}/{r_{ij}}) \cdot \beta_{iw}) \leq \widetilde{D_{i}}, \quad \forall i \in \textbf{U}, \label{eq6} \\
& x_{jk}, x_{jw}, x_{ijw} \in \{0, 1\}, \quad \forall i \in \textbf{U}, \, \forall j \in \textbf{M}.\label{eq7}
\end{align}  
%constraints -- 下面前两段为Lu已经列好的英文，Xu的解释将紧接着用中文先行给出
$\textbf{P}_1$ is the objective function. Equations~(\ref{eq4}) to (\ref{eq7}) are constraints. Equation~\ref{eq4} is the edge server deployment cost, which needs to be less than the total budget $C_{\max}$, representing the maximum allowed cost for edge server deployment. Equation~\ref{eq5} enforces the physical resource limitations of edge server $m_j$. Specifically, the total storage required by all services deployed on $m_j$, represented by $\sum_{w \in \textbf{S}} x_{jw}\cdot v_w$, must not exceed its storage capacity $V_j$. Similarly, the total computational resources required by the deployed services, given by $\sum_{w\in S_j} x_{jw}\cdot \varsigma_{w}$, must not exceed the computing capacity $\mu_j$ of $m_j$. Equation~\ref{eq6} ensures QoS for each user by requiring that the total latency incurred when accessing the requested service does not exceed the maximum tolerable delay $\widetilde{D_i}$. Equation~\ref{eq7} states the decision variables, including whether the server $m_j$ is deployed at the base station $b_k$, whether the service $s_w$ is provisioned on the server $m_j$, and whether the user $u_i$ accesses the service $s_w$ via the server $m_j$. 
...
\begin{figure*}[!t]
 \centering
    ...
    \vspace{-10pt}
    \caption{The overall architecture of MOS\textsuperscript{2}.}
    \label{fig:algo}
    \vspace{-15pt}
\end{figure*}
...
Given the fixed server locations, Stage 2 introduces the Pareto-oriented Service Provisioning (PSP) strategy, built upon the NSGA-II framework, to tackle the multi-objective optimization of provisioning cost and access delay. As illustrated in the bottom panel of Fig.~\ref{fig:algo}, this stage comprises three core algorithmic components operated sequentially. First, instead of purely random initialization, the algorithm evaluates and ranks the candidate service pool to construct an initial base vector. This hybrid initialization combines a deterministic selection of top-ranked services to ensure the retention of high-quality elite traits, with a stochastic random fill to preserve population diversity. Next, during the evolutionary optimization process, the initial population undergoes standard genetic operations, including crossover and mutation, to generate offspring. A critical feature here is the explicit constraint-handling mechanism. Offspring are subjected to a server capacity check, and any individual violating this limit is forced into a repair loop, where excess services are dynamically dropped until feasibility is restored. The repaired offspring and parent populations are then merged, evaluated, and subjected to non-dominated sorting to breed the next generation. Finally, through successive iterations, the evolutionary process converges to a Pareto optimal set. As depicted in the Pareto Front graph, this output provides a set of optimal service placement schemes that achieve the best possible trade-offs between cost minimization and latency reduction, empowering network operators to make flexible provisioning decisions based on specific operational preferences.
% =========================================================
...
\begin{algorithm}[!t]
    ...
    \KwOut{$\mathcal{S}$, $C$}
    $\mathcal{S} \leftarrow \textbf{Select}(\mathcal{M},k)$\;
...
We reformulate the server deployment model as a facility location problem, where the candidate deployment base stations correspond to potential server locations, and the users are abstracted as demand points with heterogeneous service requirements. Due to the total deployment budget $C_{\max}$ limits the overall server deployment cost, we estimate the maximum number of deployable server nodes as $\mathcal{K}^c$, where $\mathcal{K}^c=\left\lfloor C_{\max}/\min\{p_j\}\right\rfloor$, and $p_j$ denotes the deployment cost of the server $m_j$. In addition, we introduce a density threshold $\sigma_{\min}$ and assign each server a circular service coverage area with a radius of $r$ based on the density of the user. Within this radius, users are considered to be in the same region as the server, and the per-unit communication cost is assumed to be negligible. We then calculate the user density within the service range of each base station. Let $\mathcal{K}^r$ denote the number of base stations whose local user density exceeds the threshold $\sigma_{\min}$, where $\mathcal{K}^{r}=\sum _{j\in M} \mathbf{1}_{\{\sigma_{j} \geq \sigma_{\min}\}}$. Since the number of deployable edge servers is jointly constrained by the total deployment budget and the spatial distribution of user demand, we estimate the required number of servers by considering both factors, where is determined by $k=\min\{\mathcal{K}^{c}, \mathcal{K}^{r}\}$. 
Based on this estimate, we achieve an initial optimization of the deployment cost $C_d(m_j)$. We then optimize the transmission cost $C_t(m_j)$ and integrate it into a classical location problem by using the $k$-median formulation. Specifically, given a set of candidate server locations where each server can serve a certain number of users, the objective is to select $k$ base stations that deploy the servers in such a way that the total distance between each user and the nearest server is minimized. The detailed steps of the strategy are outlined in Algorithm 1.
%本段末尾再补一句，为下面第一行。
%%%%%%Xu:
...
\begin{algorithm}[!t]
    ...
    \For{$j \leftarrow 1$ \KwTo $k$}{
        ...
        \ForEach{$s_w \in C_j$}{
            ...
            Calculate the hybrid score $H(s_w)$;
        }
        Sort $C_j$ by descending $H(s_w)$;\\
...
\begin{algorithm}[!t]
    ...
    \KwIn{$\mathcal{M}$, $\mathcal{U}$, $\mathcal{S}$, $k$, $G$, $N$, $V_j$, $\alpha$, $\beta$}
    \KwOut{$\mathcal{P}^*$}
    ...
    \For{$g \leftarrow 1$ \KwTo $\mathrm{G}$}{
        ...
        \ForEach{$X' \in \mathcal{P}'$}{
            Round, reshape, repair, and evaluate $X'$;% as above\;
        }
...
\begin{figure*}[!t]
 ...
 %    \vspace{-5pt}
    ...
    \subfigure[$k=10, |U|=180$.]
	{\includegraphics[width=1.75in]{s10-u180_new.pdf}}
    \subfigure[$k=20, |U|=180$.]
	{\includegraphics[width=1.75in]{s20-u180_new.pdf}}
    \subfigure[$k=10, |U|=300$.]
	{\includegraphics[width=1.75in]{s10-u300_new.pdf}}
    \subfigure[$k=20, |U|=300$.]
	{\includegraphics[width=1.70in]{s20-u300_new.pdf}}
    \vspace{-5pt}
	\caption{Performance of server deployment (stage one) by considering different numbers of edge servers ($k$) and users ($|U|$).}
	\label{fig2}
    \vspace{-10pt}
\end{figure*}
...
\begin{figure}[!t]
	\centering
	\addtolength{\subfigcapskip}{-2pt}
        \subfigure[\# of edge servers]%server数量的讨论
	{\includegraphics[width=1.7in]{figure4-a.pdf}}
	\subfigure[convergence behavior]%迭代轮次的讨论
	{\includegraphics[width=1.7in]{figure4-b.pdf}}
    \vspace{-5pt}
	\caption{Cost comparison from different aspects.} 
	\label{fig_compare}
    \vspace{-10pt}
\end{figure}
\begin{figure*}
	...
	\addtolength{\subfigcapskip}{-2pt}
     \subfigure[\# users (100).]
        {\includegraphics[width=1.75in]{10_100.pdf}}
	\subfigure[\# users (130).]
	{\includegraphics[width=1.75in]{10_130.pdf}}
	\subfigure[\# users (150).]
	{\includegraphics[width=1.75in]{10_150.pdf}}
	\subfigure[\# users (180).]
	{\includegraphics[width=1.65in]{10_180.pdf}}
	% {\includegraphics[width=1.75in]{100users.pdf}}
	% \subfigure[\# users (130).]
	% {\includegraphics[width=1.75in]{130users.pdf}}
	% \subfigure[\# users (150).]
	% {\includegraphics[width=1.75in]{150users.pdf}}
	% \subfigure[\# users (180).]
	% {\includegraphics[width=1.65in]{180users.pdf}}
    \vspace{-5pt}
	\caption{Performance of service provisioning (stage two) by considering different numbers of users.} 
	\label{fig4}
    \vspace{-10pt}
\end{figure*}
...
In this study, a service deployment scheme is represented by a binary matrix $\textit{indiv}$. For a system containing $k$ servers and $|\mathcal{S}|$ service types, this deployment matrix has dimensions $k \times |\mathcal{S}|$, where each row corresponds to a server and each column to a service type. The element $\textit{indiv}[j, w]$ at row $j$ and column $w$ takes a value of $0$ or $1$, indicating whether service $w$ is provisioned on server $j$ ($\textit{indiv}[j, w] = 1$ denotes the service is provisioned; otherwise, $\textit{indiv}[j, w] = 0$ denotes its absence). The total service deployment cost is formulated as:
\begin{equation}
C_{p}(m_j) = \sum_{w \in \mathcal{S}} \sum_{j \in \mathcal{M}} \textit{indiv}[j, w] \cdot sc[w]
\end{equation}
To ensure that no server exceeds its predefined capacity, let $V_j$ denote the maximum number of services allowed on server $j$, enforced by the following constraint:
\begin{equation}
\sum_{w \in \mathcal{S}} \textit{indiv}[j, w] \leq V_j, \quad \forall j \in \mathcal{M}.
\end{equation}
When deployment violates capacity limits, a repair strategy randomly removes excess services by setting the corresponding $\textit{indiv}[j, w]$ to $0$ until the constraint is satisfied.

Based on that, we design a Pareto-based Service Provisioning (PSP) strategy leveraging the multi-objective optimization principles of NSGA-II, with specific improvements to its initial population generation configuration. Rather than random initialization, we implement a hybrid greedy strategy combining cost and delay considerations.  This approach: 1) builds two greedy candidate rankings per server---one prioritizing low deployment cost and
  one prioritizing low access delay; 2) fuses them via a tunable hybrid score that yields a single ranking
  balancing both objectives; and 3) selects a deterministic anchor set from the top of the ranking and fills the
   remaining capacity by stochastic sampling, producing a diversified yet high-quality seed for the evolutionary
   loop. The resulting population serves as the starting point for subsequent multi-objective optimization. To effectively evaluate and rank candidate services in terms of both cost and delay, we define a hybrid score, which integrates these two objectives into a single scalar value.
% Definition for Generate Initial Population (Hybrid Greedy)
\begin{definition}[hybrid score]\label{def:init_pop}
Let $\alpha,\beta\in[0,1]$ be the weights balancing cost and delay ($\alpha+\beta=1$), and $\varpi_j \;(0<\varpi_j\le V_j)$ the size of the deterministic anchor set selected for edge server $m_j$. The hybrid score of a candidate service deployed on $m_j$ is defined as: 
\begin{equation}
H(s_w)=\alpha w_{\mathrm{cost}}(s_w)+\beta w_{\mathrm{delay}}(s_w).
\end{equation}
where $w_{\mathrm{cost}}(s_w)$ and $w_{\mathrm{delay}}(s_w)$ denote the cost-oriented and delay-oriented scores of service $s_w$. 
\end{definition}
...
Line~1 of Algorithm~\ref{alg:init_pop_hybrid} initializes the base matrix $X^{\mathrm{base}}$ to a zero
  ...
  score $H(s_w)$ defined in Eq.~(14) for every candidate service $s_w \in C_j$, and line~8
...
To intuitively illustrate this hybrid initialization process, we take Server 0 as an example and visualize its candidate service evaluation and selection mechanisms in Fig.~\ref{fig:hybrid_init}. Specifically, Fig.~\ref{fig:hybrid_init}(a) presents the score breakdown for candidate services, demonstrating how the total hybrid score is additively composed of the cost-oriented and delay-oriented weighted components. Based on these evaluations, Fig.~\ref{fig:hybrid_init}(b) depicts the subsequent selection logic executed under a strict server capacity constraint. As shown, the highest-ranked candidates (e.g., Service 0 and Service 1) are deterministically selected to form the high-quality anchor set, guaranteeing the retention of elite traits. Simultaneously, the remaining capacity slots are filled by stochastically selecting from the lower-ranked remainder (e.g., Service 6 and Service 4), while unselected ones (e.g., Service 2) are discarded. This visualization concretely verifies the core rationale of our strategy: it successfully strikes a balance between exploitation---locking in the most cost-effective and highly demanded services---and exploration---introducing stochastic elements to maintain population diversity and effectively prevent premature convergence to local optima.
...
Each generation merges parent and offspring populations, re-sorts them, and selects the top $N$ individuals for the next generation, ensuring convergence and diversity. The iterative process terminates at the maximum generation limit, outputting: 1) a Pareto-optimal set of capacity-constrained service deployment schemes, and 2) the optimal joint cost under weighted objectives. The detailed steps of the strategy are outlined in Algorithm 2 and Algorithm 3.
%%%%第二部分开始
...
%整合优化新版
Let $\mathcal{M}$ be the set of deployed servers obtained from Stage~1,
  $\mathcal{U}$ the set of user positions,
  $\mathcal{S}$ the catalog of service types, and
  $k=|\mathcal{M}|$ the number of deployed servers.
  % Updated descriptive paragraph
...
\begin{figure}[!t]
	\centering
	\addtolength{\subfigcapskip}{-2pt}
        \subfigure[Hybrid score breakdown]% 阶段1：评分构成的讨论
	{\includegraphics[width=1.7in]{hybrid_score_breakdown.pdf}}
	\subfigure[Selection logic]% 阶段2：选择逻辑的讨论
	{\includegraphics[width=1.7in]{hybrid_selection.pdf}}
    \vspace{-5pt}
	\caption{The hybrid initialization process for candidate services on Server 0.} 
...
\begin{figure*}
    ...
    \subfigure[\# edge servers (5).]
        {\includegraphics[width=1.73in]{5_130.pdf}}
	\subfigure[\# edge servers (10).]
	{\includegraphics[width=1.73in]{10_130.pdf}}
	\subfigure[\# edge servers (15).]
	{\includegraphics[width=1.73in]{15_130.pdf}}
	\subfigure[\# edge servers (20).]
	{\includegraphics[width=1.73in]{new20_130.pdf}}
	% {\includegraphics[width=1.75in]{edgeserver5.pdf}}
	% \subfigure[\# edge servers (10).]
	% {\includegraphics[width=1.75in]{edgeserver10.pdf}}
	% \subfigure[\# edge servers (15).]
	% {\includegraphics[width=1.75in]{edgeserver15.pdf}}
	% \subfigure[\# edge servers (20).]
	% {\includegraphics[width=1.75in]{edgeserver20.pdf}}
    \vspace{-5pt}
	\caption{Performance of service provisioning (stage two) by considering different numbers of edge servers.} 
	\label{fig6}
    \vspace{-10pt}
\end{figure*}
...
\begin{figure*}
    ...
    \vspace{-5pt}
	\caption{Performance of service provisioning (stage two).} 
	\label{fig7}
    \vspace{-10pt}
\end{figure*}
...
%NSGA-II对比算法
\begin{itemize}
    \item \textbf{NSGA-II Provision (NS-P)}: The initial population is generated using random service provisioning strategies.
...
We validate the superiority of our enhanced PSP strategy by comparing scenarios with varying scales of base stations and users. Two experimental configurations are designed: 
\begin{enumerate}
    \item Fixing the number of deployed servers at 10 while incrementally increasing the user count from 100 to 180.
    \item Fixing user number at 130 while scaling server deployment from 5 to 20.
\end{enumerate}

Fig.~5 and Fig.~7 present the experimental results, where the vertical axis represents the normalized weighted total cost combining deployment cost and user delay. The PSP strategy consistently achieves the lowest weighted values across all configurations. Both GCP and GDP outperform the original NS-P strategy, which relies on random initial population generation, due to their optimized initial population construction. Furthermore, PSP enhances performance beyond GCP and GDP by integrating partial solutions from their initial populations while introducing stochastic elements to avoid local optima.

Fig.~8 displays the Pareto front solutions for the fixed-user scenario (130 users). The red curve (PSP) demonstrates superior coverage compared to other strategies in most cases, indicating that PSP achieves more optimal joint minimization of deployment cost and user delay under equivalent optimization preferences. Collectively, these experiments confirm the significant superiority of our proposed PSP strategy in service provisioning optimization.
...
\section{Conclusion}
 This paper presents MOS², a two-stage multi-objective framework for joint cost-delay optimization in mobile edge networks,
  ...
  decisions. Future work will explore joint learning-based optimization for content popularity and mobility pattern estimation, and further extend the framework to handle federated scenarios with multi-domain coordination. 
