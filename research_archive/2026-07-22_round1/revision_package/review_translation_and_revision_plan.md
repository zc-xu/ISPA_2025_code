...
来源文件：
- 回信文本：已整理到本归档的回复材料中。
- 论文 PDF：`manuscript/conference_101719_first_submission.pdf`
- 代码仓库：本 Git 仓库根目录。
...
代码依据：
- Stage I 随机局部搜索在 `main.py`。
- Stage II 编码在 `LocalSearch/nsga_service_deploy.py`。
...
代码依据：
- `ServiceRepair` 在 `LocalSearch/nsga_service_deploy.py`。
...
代码依据：
- Fig. 5/7/8 相关绘图在 `LocalSearch/compare_results.py` 和 `LocalSearch/nsga_service_deploy.py` 的可视化部分。
...
代码依据：
- `NSGA2` 在 `LocalSearch/nsga_service_deploy.py`。
...
代码依据：
- `alpha=0.5, beta=0.5` 在 `LocalSearch/nsga_service_deploy.py`。
- 确定性锚点大小参数在 `LocalSearch/nsga_service_deploy.py`。
- `SERVICE_CAPACITY_PER_SERVER = 4` 在 `LocalSearch/compute_delay.py`。
